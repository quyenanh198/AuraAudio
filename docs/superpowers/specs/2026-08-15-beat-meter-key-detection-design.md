...
| Key | `music21`'s built-in key analysis (`Stream.analyze('krumhansl')`, explicitly — **not** the bare `'key'` method name) | Already a direct dependency of `packages/musicxml`. Returns a `Key` object exposing `.correlationCoefficient`, usable directly as a confidence proxy. Verified during implementation: `Stream.analyze('key')`'s default weighting is **not** Krumhansl-Schmuckler despite that being commonly assumed — it misclassified a clean C-major scale as A minor (correlation 0.824) where `'krumhansl'` correctly resolved it to C major (0.901). Always request the algorithm by name. |
